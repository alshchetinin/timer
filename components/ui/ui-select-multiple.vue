<template>
  <Listbox v-model="selected" multiple as="div" class="group relative text-sm">
    ...
    >
      <div v-if="selected.length">
        <p v-if="selected.length > 1">
          {{ selected[0].name }}
          <span class="text-black/40"> +{{ selected.length - 1 }}</span>
        </p>
        <p v-else>{{ selected[0].name }}</p>
      </div>
      <p v-else>Любой</p>
    ...
    <div
      ...
      class="absolute left-0 z-20 mt-1 w-full bg-white"
    >
      <div v-if="isOpen" v-on-click-outside="onClickOutsideHandler">
        ...
        >
          <button
            class="flex w-full items-center gap-2 border-b px-3 py-2 text-black transition-all hover:bg-primary/5"
            @click="any"
          >
            <span>Любой</span>
          </button>
